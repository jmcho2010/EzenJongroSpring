...
public interface ProductRepository extends JpaRepository<Product, String>{
	...
	// 커스텀 쿼리
	// JPArepo : 자바 문법으로 db에 요청할수 있도록 준비되어있는
	//           메서드들을 활용해서 처리.
	//  -> 하지만 현실적으로 쿼리자체가 간단하게 나오는 경우보다 복잡하게 나오는 경우도 상당수.
	
	// @Query 어노테이션을 통해 DB에서 활용하는 쿼리들을 쓸수 있다.
	// 1. JPQL : 엔티티를 대상으로 하는 객체지향 쿼리 언어
	//    -> SQL과 문법이 거의 같음.
	//    -> 특정 DB에 종속되지 않는다는 장점이 있음.
	// @Param : 서비스단에서 호출될때 파라미터를 받기위해 쓰는 어노테이션.
...
//	List<Product> findProduct1(@Param("minPrice") BigDecimal minPrice);
	...
	// 2. Native Query
	@Query(value = "SELECT * FROM products WHERE buyprice > :minPrice", 
			nativeQuery=true)
	List<Product> findProduct1(@Param("minPrice") BigDecimal minPrice);	
	...
	// 공통점 : 쿼리 사용.
	// 차이점 : 중심이 어디에 있느냐.
	//  - JPQL은 객체중심, Native Query는 db에 더 가까운 형태.
	
	// 현업에서는 언제쓰느냐
	//  - 일반적으로는 JPQL이 DB를 안탄다는 장점이 있음.
	//  - 하지만 Native Query가 DBMS가 변경이 안된다라는 보장이 있다면
	//    JPQL보다 조회라던지 표현범위는 훨씬 넓기때문에 
	//    특정 DB의 고유 기능을 사용할떄는 Native Query가 좋다.
	...
	//  -> 서버 실행할때 문제있으면 발견됨.
	//  -> 자동완성 기능도 사용가능. 안정성 측면에서 큰 장점을 가져감.
	//  -> 기존 자바 문법과의 조합도 가능.
	//  단점 : 초기 세팅이 귀찮을수 있음.
