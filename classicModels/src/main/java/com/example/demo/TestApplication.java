...
//	String cliIp = req.getRemoteAddr();
	...
	// securityConfig에서 추가로 설정이 진행
	// oauth2Login()
	// 1. loginPage() : 소셜 로그인을 시작할수 있는 버튼들이 있는 페이지를 지정.
	// 2. defaultSuccessUrl()
	// 3. userInfoEndpoint() : 소셜 로그인 성공 후에 사용자 정보를 가져오는것에 대한 설정을 진행하는곳
	// 4. userService() : 구글이나 카카오 같은 곳에서 사용자 정보를 받아온후에 우리가 원하는 작업을 할수 있도록
	//                    후 처리를 진행하는 부분. 여기서 DB에 맞게 회원가입, 정보 업데이트.
	...
	// JWT를 활용한 로그인과정
	// 1. 기존의 ID/PW나 OAuth2를 활용한 로그인을 통해 인증에 성공.
	// 2. JWT 발급 : 보통 Authorization 헤더에 담김.
	// 3. 브라우저는 해당 JWT를 보관(로컬 스토리지)
	// 4. 요청시 쿠키 제출 : 다른기능(다른페이지 접근)을 요청할때마다 JWT 쿠키를 요청 헤더에 첨부해서 보냄.
